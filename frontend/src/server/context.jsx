...
export function StoreProvider({ children, initialSnapshot }) {
  ...
  async function account(action) {
    generation.current++;
    await queue.current;
    const result = await api.request("account/", {
      method: "POST",
      body: action,
    });
    generation.current++;
    ...
    const run = async () => {
      ...
      try {
        if (action.type === "switch") {
          if (action.user !== null)
            throw Error("Войдите в другой аккаунт с его паролем.");
        ...
        if (["order-refund", "order-cancel"].includes(action.type)) {
          ...
          await api.request(
            "../store/orders/" +
              encodeURIComponent(action.order) +
              "/" +
              method +
              "/",
            { method: "POST", body: {}, user: actor },
          );
          generation.current++;
          await refresh();
  ...
  async function uploadAttachment(file) {
    const body = new FormData();
    body.append("file", file);
    return api.request("attachments/", {
      method: "POST",
      body,
      user: currentUser.current,
    });
  ...
  return (
    <Context.Provider
      value={{
        state: data.state,
        me: data.state.users.find((u) => u.id === data.state.active),
        act,
        account,
        uploadFile,
        uploadAttachment,
        reset: () => act({ type: "preferences-reset" }, "Настройки сброшены"),
        notify: setNotice,
      }}
    >
      {children}
